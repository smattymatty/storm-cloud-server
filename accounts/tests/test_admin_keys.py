...
class AdminAPIKeyListTest(StormCloudAdminTestCase):
    ...
    def test_admin_list_keys_filter_by_user_id(self):
        """Admin can filter keys by user_id."""
        # GET /api/v1/admin/keys/?user_id={user1.id}
        # Verify only user1's keys returned
        user1 = UserWithProfileFactory()
        user2 = UserWithProfileFactory()
        key1 = APIKeyFactory(user=user1)
        key2 = APIKeyFactory(user=user2)

        response = self.client.get(f'/api/v1/admin/keys/?user_id={user1.id}')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        # Verify only user1's keys are in response
    ...
    def test_admin_list_keys_filter_by_is_active(self):
        ...
        active_key = APIKeyFactory(user=user)
        revoked_key = APIKeyFactory(user=user, revoked=True)
    ...
    def test_admin_list_keys_includes_username(self):
        """Key list includes associated username."""
        # GET list
        # Verify response includes username for each key
        user = UserWithProfileFactory(username='testusername')
        APIKeyFactory(user=user)

        response = self.client.get('/api/v1/admin/keys/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        # Verify at least one key has username field
        self.assertTrue(any('username' in k or 'user' in k for k in response.data['keys']))
...
class AdminAPIKeyRevokeTest(StormCloudAdminTestCase):
    ...
    def test_admin_revoke_any_users_key_succeeds(self):
    ...
    def test_non_admin_cannot_revoke_others_keys(self):
        """Non-admin cannot revoke other users' keys."""
        # Authenticate as user2 (non-admin)
        # POST revoke user1's key
