...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--user-id',
            type=int,
            help='Sync specific user only (default: all users)'
        )
        parser.add_argument(
            '--dry-run',
            action='store_true',
    ...
    def _display_stats(self, stats, verbosity):
        ...
        if stats['missing_in_db'] > 0:
            self.stdout.write(self.style.WARNING(
                f"⚠ Missing in DB: {stats['missing_in_db']}"
            ))
        if stats['orphaned_in_db'] > 0:
            self.stdout.write(self.style.WARNING(
                f"⚠ Orphaned in DB: {stats['orphaned_in_db']}"
            ))
        ...
        if stats['records_deleted'] > 0:
            self.stdout.write(self.style.SUCCESS(
                f"✓ Records deleted: {stats['records_deleted']}"
            ))
        ...
        # Success message
        if stats['records_created'] == 0 and stats['records_deleted'] == 0 and not stats['errors']:
