...
class FileCreateTest(StormCloudAPITestCase):
    ...
    def test_create_empty_file_succeeds(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response.data["detail"], "File created")
        self.assertEqual(response.data["path"], unique_file)
        self.assertEqual(response.data["size"], 0)
        self.assertFalse(response.data["is_directory"])

    def test_create_file_with_content_type(self):
        """Creating file should detect content type from extension."""
        self.authenticate()
        import uuid

        unique_file = f"document-{uuid.uuid4().hex[:8]}.json"
        response = self.client.post(f"/api/v1/files/{unique_file}/create/")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_create_existing_file_returns_409(self):
        ...
        self.authenticate()
        self.client.post("/api/v1/files/existing.txt/create/")
        response = self.client.post("/api/v1/files/existing.txt/create/")
        self.assertEqual(response.status_code, status.HTTP_409_CONFLICT)
        self.assertEqual(response.data["error"]["code"], "ALREADY_EXISTS")

    def test_create_file_auto_creates_parent_dirs(self):
        """Creating file in non-existent directory should create parents."""
        self.authenticate()
        import uuid

        unique_path = f"deep/nested/dir-{uuid.uuid4().hex[:8]}/file.txt"
    ...
    def test_create_file_sets_encryption_method(self):
        """Created file should have encryption_method set (ADR 006)."""
        from storage.models import StoredFile

        self.authenticate()
        import uuid

        unique_file = f"encrypted-test-{uuid.uuid4().hex[:8]}.txt"
        response = self.client.post(f"/api/v1/files/{unique_file}/create/")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response.data["encryption_method"], "none")

        # Verify in database
        stored_file = StoredFile.objects.get(owner=self.user, path=unique_file)
        self.assertEqual(stored_file.encryption_method, "none")
