...
class BulkOperationServiceTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.service = BulkOperationService(user=self.user, backend=self.backend)
    ...
    def test_delete_single_file(self):
        ...
        result = self.service.execute(operation='delete', paths=['test.txt'], force_sync=True)
        ...
        self.assertFalse(StoredFile.objects.filter(owner=self.user, path='test.txt').exists())
    ...
    def test_delete_directory_recursive(self):
        ...
        result = self.service.execute(operation='delete', paths=['mydir'], force_sync=True)
    ...
    def test_delete_nonexistent_file(self):
        ...
        result = self.service.execute(operation='delete', paths=['missing.txt'], force_sync=True)
    ...
    def test_delete_partial_success(self):
        ...
        self.assertFalse(StoredFile.objects.filter(owner=self.user, path='good.txt').exists())
    ...
    def test_copy_single_file(self):
        ...
        self.assertEqual(StoredFile.objects.filter(owner=self.user).count(), 3)  # source, dest dir, copy
