...
class IndexSyncService:
    ...
    def _sync_organization(
        ...
        db_files = {
            f.path: f for f in StoredFile.objects.filter(organization=org)
        }
        stats.files_in_db = len(db_files)
        ...
        if mode in ['sync', 'full']:
            for path in missing_in_db:
                ...
                if not dry_run:
                    try:
                        obj, created = self._create_shared_db_record(org, path, file_info)
                        if created:
                            stats.records_created += 1
                    except Exception as e:
            ...
            try:
                ...
                if detected_method == 'server':
                    ...
                    try:
                        ...
                        logger.warning(f"Cannot decrypt shared {path} for size detection")
                        original_size = file_info['size']
                        encrypted_size = None
                    except Exception as e:
                        logger.error(f"Error reading shared {path} for encryption detection: {e}")
