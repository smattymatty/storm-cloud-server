...
class BulkOperationService:
    ...
    def execute(
        ...
        if len(paths) > self.MAX_PATHS:
            raise ValueError(
                f"Maximum {self.MAX_PATHS} paths per request "
    ...
    def _execute_move(self, path: str, destination: str) -> BulkOperationResult:
        ...
        try:
            ...
            dest_full = f"{self.user_prefix}/{normalized_dest}" if normalized_dest else self.user_prefix
            ...
            new_parent_path = str(Path(new_relative_path).parent) if "/" in new_relative_path else ""
            if new_parent_path == ".":
                new_parent_path = ""
            ...
            return BulkOperationResult(
                path=path,
                success=True,
                data={'new_path': new_relative_path}
            )
    ...
    def _execute_copy(self, path: str, destination: str) -> BulkOperationResult:
        ...
        try:
            ...
            if db_file.size > 0:
                ...
                storage_used_bytes = StoredFile.objects.filter(
                    owner=self.user
                ).aggregate(total=Sum('size'))['total'] or 0
                
                # Check if quota is set and would be exceeded
                if user_profile.storage_quota_bytes > 0:
                    if storage_used_bytes + db_file.size > user_profile.storage_quota_bytes:
